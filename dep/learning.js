// TODO: figure out what's causing the following error message
// SyntaxError: missing ; before statement RLOs.json:1:7
...
function showRLOs() {
  ...
  function displayRLOs (rloList) {
    ...
    _.forEach(rloList, function(rlo) {
      ...
      console.log( 'url:', rlo.url, 'urlCode:', rlo.urlCode, 'urlDemo:', rlo.urlDemo )
      if (rlo.url !== undefined && rlo.url.length > 4) {
        var type = learning.types[rlo.type].toLowerCase();
        links += ("~ <a href='" + rlo.url + "'>" + type + "</a> ");
      }
      if (rlo.urlCode !== undefined && rlo.urlCode.length > 4) {
        links += ("~ <a href='" + rlo.urlCode + "'>source code</a> ");
      }
      if (rlo.urlDemo !== undefined && rlo.urlDemo.length > 4) {
        links += ("~ <a href='" + rlo.urlDemo + "'>demonstration</a> ");
      }
      if (links.length > 4) { links += "~"; } 
      else { links = " ~ COMING SOON ~ "; }
      
      console.log(links);
      
      
      e.find('.rlo-links').html(links);
